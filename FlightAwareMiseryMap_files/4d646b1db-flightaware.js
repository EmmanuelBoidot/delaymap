...
/* flightaware.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */
...
/* jquery.min.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */
...
/* jquery.custom.min.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */
...
/* jquery.hint.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */
/**
* @author Remy Sharp
...
/* jquery.bgiframe.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */
...
$.fn.bgiframe = ($('body').hasClass("ie6") ? function(s) {
    s = $.extend({
...
/* jquery.tooltip.min.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */
/*
...
/* jquery.tipsy.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */
(function($) {
...
/* jquery.cookie.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */
/*!
...
$.cookie.defaults.path = '/'; $.cookie.defaults.expires = 3650; $.cookie.defaults.domain = ".flightaware.com";


/* jquery.numeric.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */
/******************************************************************************/
/* jQuery Numeric 0.1 *********************************************************/
...
/* jquery.universalbits.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */


$(function()
{
    // Specifying the element is faster for older browsers
    var $dropmenu = $('ul.menulist');
    var $dropdowns = $('li.topMenu');
     ...
     */
    if ('ontouchstart' in document.documentElement || ('ontouchstart' in window) || (window.DocumentTouch && document instanceof DocumentTouch))
    {
            ...
            this.addEventListener('touchstart', function(e)
            {
                if (e.touches.length === 1)
                {
                    ...
                    if (!$this.hasClass('hover'))
                    {
                        ...
                        if (e.target === this || e.target.parentNode.parentNode === this || e.target.parentNode === this)
                        {
                            e.preventDefault();
                        }
...
/* CUSTOM JQUERY AND JAVASCRIPT FOR FA TEMPLATE */
...
function foreignIdentTest(ident) {
    //if first char is a number
    var numbers = /[0-9]/;
    if (numbers.test(ident.charAt(0))) {
      if (ident.length <= 5) {
        //if string is then less than 4 if you return all the numbers:
        var identNoDigits = ident.replace(/[0-9]/g, '');
        if (identNoDigits.length < 4) return false;
      }
    }
    return true;
}
...
function validateIdent(ident) {
    var newIdent = '', alphanum = /^[a-zA-Z0-9#]+$/, digits = /^[0-9]+$/, alphabet = /^[A-Z]+$/, fleet = /^\w+\*$/, possibleAirlineFlight = true;

    if (fleet.test(ident)) return "FLEET";

    newIdent = ident.toUpperCase();
    //remove spaces, dashes and periods
    newIdent = newIdent.replace(/[\s#-]/g, '');

    //replace period separately, because it replaces exclamation marks too (seriously)
    //escaping the period does not fix the issue
    newIdent = newIdent.replace(/[\.]/g, '');

    if (!alphanum.test(newIdent) || newIdent.length < 2 || newIdent.length > 9) return 'INVALID';

    if(!foreignIdentTest(newIdent)) {
        newIdent = 'N' + newIdent;
        possibleAirlineFlight = false;
    }

    var identPrefix = newIdent.substring(0, 3);
    var identSuffix = newIdent.substring(3);
    var identPrefixTwo = newIdent.substring(0, 2);

    // in case user entered short airline code: "CO" instead of "COA"
    if (alphabet.test(identPrefixTwo) && digits.test(newIdent.substring(2))) {
        if (airlineIATA[identPrefixTwo] != undefined) {
                identPrefix = airlineIATA[identPrefixTwo];
                identSuffix = newIdent.substring(2);
                newIdent = identPrefix + identSuffix;
        }
    }

    var separator = ' - ';
    if (possibleAirlineFlight) {
      for (var i=0,len=airlines2.length;i<len;i++) {
        // we can rely on every airline record having the code after the separator
        var substrStart = airlines2[i].lastIndexOf(separator) + separator.length;

        //only if the prefix matches an airline would we string the leading zeroes from the suffix
        if (identPrefix == airlines2[i].substring(substrStart)) {
          // remove zeroes from start of identSuffix string if there are any.
          newIdent = identPrefix + identSuffix.replace(/^[0]+/g,"");
        }
      }
    }
...
$(document).ready(function(){
    // define cache for less DOM traversing
    var $airline_name_quickTracker = $(document.getElementById('airline_name_quickTracker')),
        $flightTrackerIdentError = $(document.getElementById('flightTrackerIdentError')),
        $ident_suffix_quickTracker = $(document.getElementById('ident_suffix_quickTracker')),
        $airline_name = $(document.getElementById('airline_name')),
        $airport = $('#airport'),
        $airportCodeAutocomplete = $('#airport,#airportCode'),
        $airport_name_or_city = $(document.getElementById('airport_name_or_city')),
        $ident_suffix = $(document.getElementById('ident_suffix')),
        $ident_Nav = $(document.getElementById('ident'));

	// If the input val = title then we most likely have IE10 hint issues from clicking "Back"
	// clear out the values to re-assign the hint
	if (/*@cc_on !@*/false && (document.documentMode === 10)) {
		if ($('input[title][type="text"]').val() == $('input[title][type="text"]').attr('title')) {
			$('input[title][type="text"]').val("");
		}
	}
	
    // find all the input elements with title attributes
    $('input[title][type="text"]').hint();

    //find all inputs and add autocorrect=off
    $('input[type="text"]').attr('autocorrect','off');
    // feature detection

    // this needs to be manually added, the above won't cause the hint action on it.
    $('#forgotFlightNumber input[title != ""]').hint();

    $("span.bubble[title]").tipsy();
    $(".hint[title]").tipsy();
    $(".user_link[title]").tipsy();
    $(".badges a").tipsy();


    $("#navBar").find('ul').find('li').css('zIndex', 80);
    
    $("#listMenuRoot").menuAim({
        // Function to call when a row is purposefully activated. Use this
        // to show a submenu's content for the activated row.
        activate: function(a){$(a).addClass("hover").find('.titleLink').addClass("highlighted");},
        // Function to call when a row is deactivated.
        deactivate: function(a){$(a).removeClass("hover").find('.titleLink').removeClass("highlighted");},
        // flag to close menu on mouse out.
        closeMenuOnMouseLeave: true
    });

    //create Searching... element and add to body (so that it can be positioned absolutely)
    var fauxListWrapper = $("<div/>").hide().css("position", "absolute").appendTo(document.body).addClass('ac_results').addClass('fauxListWrapper');          		
    var fauxList = $("<ul/>").appendTo(fauxListWrapper);
    var fauxListElement = $("<li/>").appendTo(fauxList).html('Searching...');

    // 	show js-enabled airline search text field: it's default display: none style will hide it from js-disabled browsing. 
    $('.jsEnabledAirlineField').show();

    // unfortunately numeric has a bug which breaks using the Return key to submit the form.
    //$("#ident_suffix").numeric();
    ...
    $('#FacebookShare').click(function() {
      if (user['id'] !== "")
        $.ajax({url: "/ajax/badges/award_socialite_badge.rvt"});
    });
    ...
    $airline_name_quickTracker.autocomplete({
        source: airline1or2,
        minLength: 1,
        delay: 0,
        autoFocus: true
	...
	// NOTE: If you change this, make sure to also change the autocomplete on
	// the FBO Toolbox description/purchase page - fbo_buy.js
    $airportCodeAutocomplete.autocomplete({source:airportCodes,
        minLength: 1,
		delay: 0,
        select: function(event, item) {
            var inputID = this.id;
            //automatically submit form on select, after waiting 200ms.
            setTimeout(function() {$('#'+inputID).parent('form').submit();},200);
            
        }
      });

    $('#track').submit(function() {
        $flightTrackerIdentError.css('display','none');

        msg = validateIdent($ident_Nav.val());
        if (msg === "FLEET") {
          location.href = "/live/fleet/" + $ident_Nav.val().replace("*","");
          return false;
        } else if (msg == 'INVALID') {
          $ident_Nav.focus();
          $flightTrackerIdentError.show();
          $flightTrackerIdentError.html('&nbsp; &ndash;&gt; Invalid flight # "' + $ident_Nav.val() + '"');
        } else {
          location.href = '/live/flight/' + msg;
        }

       return false;
    });

    $('#home-main-search-ident, #topbar-search, #searchform, #quickTrackerCommercial').on('submit', function(e) {
        e.preventDefault();
        var elID = e.target.id;
        switch (elID) {
            case "home-main-search-ident" :
            case "topbar-search" :
                var finderAirlineVal = 'input[name="airline"]';
                var finderSuffixVal = 'input[name="ident_suffix"]';
                break;
            case "searchform" :
                var finderAirlineVal = 'input[name="q"]';
                var finderSuffixVal = 'input[name="ident_suffix"]';
                break;
            case "quickTrackerCommercial" :
                var finderAirlineVal = 'input[name="q"]';
                var finderSuffixVal = 'input[name="ident_suffix"]';
                break;
        }
        var $input_airline = $(this).find(finderAirlineVal)
        var $input_ident_suffix = $(this).find(finderSuffixVal)

        if ($input_airline.val().length < 3) {
            alert('Please select a valid airline name');
            $input_airline.focus();
            return false;
        }

        if ($input_ident_suffix.val().length < 1) {
            alert('Please provide a flight number');
            $input_ident_suffix.focus();
            return false;
        }
        ...
        if ($input_airline.val().lastIndexOf(searchStr) == -1) {

            // in this case the user may have typed quicker than the airline list appears
            // try to find airline using passed string.

            var stringToFind=new RegExp($input_airline.val().toLowerCase());
            var joinstrToFind=new RegExp($input_airline.val().replace(/\s/g, "").toLowerCase());

            for (var i in airlines2) {
                if (stringToFind.test(airlines2[i].toString().toLowerCase())) {
                    var substrStart = airlines2[i].lastIndexOf(searchStr) + searchStr.length;
                    code = airlines2[i].substring(substrStart);
                    break;
                } else {
                    if (joinstrToFind.test(airlines2[i].toString().toLowerCase())) {
                        var substrStart = airlines2[i].lastIndexOf(searchStr) + searchStr.length;
                        code = airlines2[i].substring(substrStart);
                        //break;
                    }
            ...
            var substrStart = $input_airline.val().lastIndexOf(searchStr) + searchStr.length;
            code = $input_airline.val().substring(substrStart);
        }

        // the ident built here may be correct, and if it's not it will get validated 
        // so we can skip the ajax call to validateIdent 
        location.href = '/live/flight/' + code + $input_ident_suffix.val();

        return false;
    });

    $('#airportInfoButton').click(function() { location.href = '/live/form.rvt?airport=' + $airport.val() + '&airport_name_or_city=' + $airport_name_or_city.val() + '&airportInfo=1'; });

    $('a button.button[type=button]').click(
		function(e) {
			e.preventDefault();
			var $this = $(this);
			var linkHref = $this.parent().attr('href');
			var linkId = $this.parent().attr('id');
			// Caused conflicts, so we specify what it shouldn't be.
			if (linkId != 'aircraft_rem' && linkId != 'flight_rem') {
    ...
    $('#airportTrackForm').submit(function() {
        ...
        if ($airport_name_or_city.attr('title') != $airport_name_or_city.val() && $airport_name_or_city.val().length > 7) {     
            ...
            if($airport_name_or_city.val().lastIndexOf(searchStr) !== false) {
                var substrStart = $airport_name_or_city.val().lastIndexOf(searchStr) + searchStr.length;
        ...
        if (airportCode === "") {
            ...
            location.href = '/live/airport/' + airportCode;
        }

        return false;
    });

    // also hide "Searching..." if any airport name feild loses focus. 
    $('.airportNameField').blur(function() { fauxListWrapper.hide(); });

    //the select arrow graphic must have an id that relates to its list.
    $('.selectArrow').click( function() {
        //if the select arrow has an id, as there is an element on the page that is the id + 'Container', toggle that element
        if($('#' + $(this).attr('id') + 'Container').length > 0) $('#' + $(this).attr('id') + 'Container').toggle();
    });

    $('.genericSelectList').click(function () {
        var split = $(this).attr('id').split(':');
        $('#'+split[0]).val(split[1]);
    });
	
	// For flight pages or flight history, clicking on the row goes to that specific flight
	//  metaKey and ctrlKey are click modifiers to open in a new tab
	$(document).on('click','.rowClickTarget',function(e) {
...
$.cookie.defaults.expire = 3650;

function exactCode(str, arr) {
	var exactCodeR = new RegExp("\\s" + regEscape(str.toUpperCase()) + "\\w*$"),
		matches = [];
	for (var x = 0; x < arr.length; x++) {
		if (exactCodeR.test(arr[x])) matches.push(arr[x]);
	}
	if (matches.length && str.length > 1) return matches;
	else return false;
}

function highLightMyAlertsLinkInHeader() {
    $('#MyAlerts').effect('highlight',{color:'#F3ED26'},900);
}

//Unified method of validating airport codes using JSONP (this uses the CDN)
function validateAirportCodeJSONP(val,id) {
    $.ajax({
        type: 'GET',
        url: airportValidateUrl,
        data: { 'data' : val },
        dataType: 'JSONP',
        crossDomain: true,
        jsonpCallback: id+'_Code',
        cache: true,
        success: function (data) {
            if (data.airportcode == 'INVALID') {
                force_valid_state(id + "_status", false);
            } else {
                $('#' + id).val(data.airportcode);
...
// get JSONP results of Airports
// this plugin handles deprecated callbacks gracefully resulting in less warnings
function airportsJSONP(val,callback,rV) {
	$.jsonp({
		url: airportNamesUrl,
		callbackParameter: "callback",
		callback: callback,
		data: {'locale':user.locale,'q':val},
		pageCache: true,
		success: function(data, textStatus, xOptions) {
			var cities = [];
			for (var i=0;i<data.aList.length; i++) {
				if (data.aList[i].city_name == '') {
					continue;
				} else {
					cities.push(data.aList[i].city_name);
				}
			}
			rV(cities);
		}
...
/* airline1or2.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */
function regEscape(text) {
	return text.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&");
}

function airline1or2(event,item) {
	// Check the begining of the words if the length = 1
	var valu = new RegExp("\\b" + event.term.toLowerCase());
	var airli = [];
	for (var i in airlines) {
		if (valu.test(airlines[i].toString().toLowerCase())) {
			airli.push(airlines[i]);
			continue;
		} else {
			//Skip this if length = 1 since there is nothing to join or replace.
			if (event.term.length != 1) {
				var joinstrToFind=new RegExp("\\b" + regEscape(event.term.replace(/\s/g, "").toLowerCase()));
				if (joinstrToFind.test(airlines[i].toString().toLowerCase())) airli.push(airlines[i]);
			}
		}
	}
	if (airli.length <= 1) {
		for (var i in airlines2) {
			var airliregexp = new RegExp("\\b" + airli.toString().toLowerCase());
			if ((airliregexp.test(airlines2[i].toString().toLowerCase()) || airli == 'Continental Air Lines (United) - COA') && airli != '') {
				continue;
			}
			if (valu.test(airlines2[i].toString().toLowerCase())) {
				airli.push(airlines2[i]);
				continue;
			} else {
				//Skip this if length = 1 since there is nothing to join or replace.
				if (event.term.length != 1) {
					var joinstrToFind=new RegExp("\\b" + regEscape(event.term.replace(/\s/g, "").toLowerCase()));
					if (joinstrToFind.test(airlines2[i].toString().toLowerCase())) airli.push(airlines2[i]);
				}
			}
		}
	}
...
/* jquery.validate.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */
/**
...
/* fa.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */
...
function DoLoginBox() {
	var boxLi = $("#LoginBoxItem");

	// If the screen is narrow, then hide the slogan as well
	var topSlogan = ($("#topContainer").width() < 1080) ? $("#topSlogan") : null;
	
	$("#topMenuList li").not(boxLi).add("#headerExtras").add(topSlogan).hide();	
	boxLi.addClass("loginShown");	
	boxLi.add("#LoginBox").fadeIn(75);
...
function NoLoginBox() {
	var boxLi = $("#LoginBoxItem");
	boxLi.removeClass("loginShown");
	boxLi.add("#LoginBox").hide();
	$("#topMenuList li").not(boxLi).add("#headerExtras").add("#topSlogan").fadeIn(75);	
}
	
function ajax_lookup_to_div(target_div,ajax_url,message,request_vars) {
  if (typeof target_div == "undefined"){ return; }
...
function ajax_lookup_to_field(field_name,ajax_url,onupdate) {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $('#' + field_name).val(dataTrim);
      if (typeof onupdate != "undefined") {
        if ( onupdate.indexOf('(') == -1 ) onupdate = onupdate + '()';
        eval(onupdate);
      }},
      error: function(data) {
        $('#' + field_name).val("Error");}
  }); 
}

function ajax_validate_field(field_name,ajax_url,onupdate) {
  ajax_url = '/ajax/' + ajax_url + '.rvt?data=' + $('#' + field_name).val();
...
function debug_toggle (element) {
  ...
  if (document.getElementById){
    ...
    for (i=0;i<x.length;i++){
      if (x[i].className == 'debug_detail') x[i].style.display='none';
    } 
  }
...
function fa_form_update (fieldname) {
  var userlist = document.getElementById(fieldname).value;
  var limit    = document.getElementById(fieldname+'_limit').value;
...
function fa_form_delete_item (fieldname, item) {
  var userlist = document.getElementById(fieldname).value.split(' ');
  var newlist = '';

  for ( var i in userlist ) {
    var uid = userlist[i];
    if (uid != item) newlist = newlist + uid + ' ';
  }
...
function fa_form_add_item (fieldname) {
  ...
  if (limit != 1) document.getElementById(fieldname+'_add').value = ''; 
}
...
function restarttimeouthandler() {
	if (typeof timeoutquant !== "undefined") {
		clearTimeout(timeoutquant);
	}
	if (typeof nTime !== "undefined") {
		timeoutquant = setTimeout(function () { location.reload(); }, nTime);
	}
}


// Page header JS
function openShareWindow(url,id,width,height,scrollbars) {
	popups = window.open(url,id,'toolbar=0,location=0,scrollbars='+scrollbars+',statusbar=0,menubar=0,resizable=0,width='+width+',height='+height+',left=20,top=40');
	window.popups.focus();
}
...
$(function(){
	// Header drop down
	var dropdownFetched = {};
	var fetchRecentActivity = function(elem) {
		// Map id to url
		var id = $(elem).attr("id");
		var url = ({
			"mainDropDown": "/ajax/home/recent.rvt?type=dropdown",
			"myFaDropDown": "/ajax/home/myaircraft.rvt"
		})[id];

		// If we don't have an ajax url, or we've already fetched this content, punt
		if (!url || dropdownFetched[id] || !$(elem).data("ajax")) {
			return;
		}
		
		dropdownFetched[id] = true;
		$.ajax({
			url: url,
			dataType: "html"
		}).done(function(data) {
			if (data !== "") {
				$(elem).find(".dropDownPlaceholder").replaceWith(data);
				$(elem).find(".dropDownRecent").on("click", "tr[data-click-url]", function() {
					window.location.href = $(this).attr("data-click-url");
				});
			} else {				
				$(elem).find(".dropDownPlaceholder").remove();
				$(elem).find(".topInfoWrapper").css("border-bottom", "none");
			}
		});
	};

	// For touch devices, the dropdown link is just a trigger
	if ('ontouchstart' in document.documentElement || ('ontouchstart' in window) || (window.DocumentTouch && document instanceof DocumentTouch)) {
		var dropDownActive = false;
		$(".topDropDownLink").click(function(e) {
			e.preventDefault();
			
			$(this).next(".topDropDown").toggleClass("hover");
			if (dropDownActive) {
				$(".topDropDownLink").parent().addClass("noHover");
			} else {
				$(".topDropDownLink").parent().removeClass("noHover");
				fetchRecentActivity($(this).closest(".topDropDown"));
			}
		...
		$(".topDropDownLink").hover(function() {
			// If we hover over the activation links, then we want to cancel any other active dropdowns
			if (timeOutObj) {
				clearTimeout(timeOutObj.timeout);
				if (timeOutObj.elem !== this) {					
					$(timeOutObj.elem).next(".topDropDown").removeClass("hover");
				}
				timeOutObj = null;
			} 

			$(this).next(".topDropDown").addClass("hover");
			fetchRecentActivity($(this).next(".topDropDown"));
		}, function() {
			var elem = this;			
			timeOutObj = {
				elem: elem,
				timeout: setTimeout(function() {
					$(elem).next(".topDropDown").removeClass("hover");
				}, 600)
			};			
		});
		...
		$(".topDropDown").on("mouseover", function() {
			if (timeOutObj) {
				clearTimeout(timeOutObj.timeout);
				timeOutObj = null;
			}
			...
			$(this).addClass("hover");
			fetchRecentActivity($(this).closest("#mainDropDown"));
		});		
		$(".topDropDown").on("mouseleave", function() {
			$(this).removeClass("hover");
		});
	}
	
	
    $('#airport_name_or_city').autocomplete({
		source: function(req,add) {
			airportsJSONP(req.term,this.element[0].name,add);
		},
        minLength: 3,
        delay: 0,
        autoFocus: true,
        ...
        select: function(event, item) {
            var airp = item.item.label;
            var airportStringSplit = airp.toString().match(/ - ([A-Z0-9]+)$/);
            if (airportStringSplit == null) $('#airport').val(item.item.label);
            else $('#airport').val(airportStringSplit[1]);
            //Use this timeout to avoid submiting form before the airport code is
            //inserted into the proper field.  Timeout set to 5ms.
            setTimeout(function() {$('#airportTrackForm').submit();},5);
        }
    });
	...
	$('#share_link').validate({
		...
		messages: {
			recipient: 'Please fix',
			sender: 'Please fix'
		},
		submitHandler: function(form) {
			var scriptUrl = '/ajax/share_email.rvt';
			var dataStr = $('form#share_link').serialize();
			$.ajax({
				type: 'POST',
				url: scriptUrl,
				data: dataStr,
				success: function(msg) {
					$('#emailShareSuccessContentWrapper').html(msg);
				}
			});
		}
	});
...
/* jquery.menu-aim.js:4d646b1db retrieved and parsed as by emit.rvt by elaan.hou.flightaware.com */
/**
